...
for fits_file in file_names:
    ...
    percents = np.percentile(file_image, q=[1, 99.7])

    # norm_img = mcolors.TwoSlopeNorm(vcenter=0.0, vmin=percents[0], vmax=percents[1])
    # norm_img = mcolors.LogNorm(vmax=percents[1])
    ...
    plt.imshow(file_image, origin='lower', norm=norm_img, zorder=-1)
    ...
    add_scalebar(axs, 15 * u.arcsec, label='$15\,\mathrm{arcsec}$', color='white', fontproperties=tick_prop, size_vertical=0.75)

    axs.annotate(text=f'$\mathrm{{ID}}:$ ${id_name:08,d}$'.replace(',', '$\,$'), xy=(0.016, 0.89),
