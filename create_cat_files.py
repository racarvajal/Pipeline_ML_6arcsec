...
mag_cols_for_colours = ['gmag', 'rmag', 'imag', 'zmag', 'ymag', 'Jmag', 'Hmag', 'Kmag', 'W1mproPM', 'W2mproPM', 'W3mag', 'W4mag']
mag_names_short      = {'gmag': 'g', 'rmag': 'r', 'imag': 'i', 'zmag': 'z', 'ymag': 'y', 
                        'Jmag': 'J', 'Hmag': 'H', 'Kmag': 'K', 'W1mproPM': 'W1', 
                        'W2mproPM': 'W2', 'W3mag': 'W3', 'W4mag': 'W4'}
...
if run_HETDEX_flag:
    ...
    if save_HETDEX_flag:
        ...
        cat_final_non_imp_HETDEX_df.to_parquet(gv.cat_path + gv.file_non_imp_HETDEX.replace('.h5', '.parquet'), engine='fastparquet')
        clean_cat_final_HETDEX_df.to_parquet(gv.cat_path + gv.file_HETDEX.replace('.h5', '.parquet'), engine='fastparquet')
        
        cat_final_non_imp_HETDEX_df.loc[:, mag_cols_for_colours].to_parquet(gv.preds_path + 'HETDEX_mags_non_imputed.parquet', engine='fastparquet')
        clean_cat_final_HETDEX_df.loc[:, mag_cols_for_colours].to_parquet(gv.preds_path + 'HETDEX_mags_imputed.parquet', engine='fastparquet')
...
if run_S82_flag:
    ...
    if save_S82_flag:
        ...
        if run_S82_full:
            ...
            cat_final_non_imp_S82_df.to_parquet(gv.cat_path + gv.file_non_imp_S82.replace('.h5', '.parquet'), engine='fastparquet')
            clean_cat_final_S82_df.to_parquet(gv.cat_path + gv.file_S82.replace('.h5', '.parquet'), engine='fastparquet')
            cat_final_non_imp_S82_df.loc[:, mag_cols_for_colours].to_parquet(gv.preds_path + 'S82_mags_non_imputed.parquet', engine='fastparquet')
            clean_cat_final_S82_df.loc[:, mag_cols_for_colours].to_parquet(gv.preds_path + 'S82_mags_imputed.parquet', engine='fastparquet')
        if not run_S82_full:
...
if run_COSMOS_flag:
    ...
    for col in mJy_cols_COSMOS:
        try:
            COSMOS_initial_tab[col] = COSMOS_initial_tab[col].filled(np.nan)
        except:
            pass

    for col in mJy_cols_COSMOS:
        COSMOS_initial_tab[col + '_AB'] = COSMOS_initial_tab[col].to(u.mag(u.AB))
    ...
    radio_cols_COSMOS   = ['Flux_COSMOSVLA3']
    mags_COSMOS_df      = COSMOS_initial_tab[magnitude_cols].to_pandas()
    flxs_COSMOS_df      = COSMOS_initial_tab[radio_cols_COSMOS].to_pandas()
    ...
    # Add radio measurements to final table
    imputed_COSMOS_df['Flux_COSMOSVLA3']    = COSMOS_initial_tab['Flux_COSMOSVLA3']
    imputed_COSMOS_df['Flux_COSMOSVLA3_AB'] = COSMOS_initial_tab['Flux_COSMOSVLA3_AB']
    ...
    for col in radio_cols_COSMOS:
        imputed_COSMOS_df.loc[:, col]     = flxs_COSMOS_df.loc[:, col].fillna(np.float32(flx_cols_lim_5sigma[col]), inplace=False)
        imputed_COSMOS_df.loc[:, col]     = imputed_COSMOS_df.loc[:, col].mask(imputed_COSMOS_df.loc[:, col] <\
                                                                           flx_cols_lim_5sigma[col], 
                                                                           flx_cols_lim_5sigma[col], inplace=False)
        non_imputed_COSMOS_df.loc[:, col] = flxs_COSMOS_df.loc[:, col]
